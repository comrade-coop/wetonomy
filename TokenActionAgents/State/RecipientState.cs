...
namespace Wetonomy.TokenActionAgents.State
{
    ...
    public delegate IList<object> TriggeredAction<T>(RecipientState<T> state, AbstractTriggerer message) where T: IEquatable<T>;
    public class RecipientState<T> where T: IEquatable<T>
    {
        ...
        public Dictionary<(string, Type), TriggeredAction<T>> TriggererToAction;
        ...
        {
            TriggeredAction<T> func = state.TriggererToAction[(sender, message.GetType())];
